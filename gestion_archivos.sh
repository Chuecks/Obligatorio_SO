...
operacion_aleatoria() {
    ...
    # Solicitar permiso a través de la pipe y esperar respuesta de `main.sh`
    echo "Solicitud de operación en $archivo desde terminal $terminal_id" > "$pipe"
    if read -r permiso < "$pipe"; then
        echo "Recibido permiso: $permiso - Ejecutando operación en $archivo desde $terminal_id"
        
        # Bloqueo con mutex
        exec 200>"$mutex"
        flock 200

        # Realizar la operación y registrar en el pipe
        case $operacion in
            0) 
        ...
        # Liberar el mutex y agregar pausa breve
        flock -u 200
        sleep 0.5  # Pausa corta para evitar saturación
    fi
}
...
while true; do
    ...
    # Crear los archivos iniciales si no existen
    ...
    sleep 1  # Pausa entre iteraciones para evitar saturación de operaciones
done
